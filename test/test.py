...
def module_setup(request, device, app_dir, artifact_dir):
    def module_teardown():
        device.run_ssh('ls -la /var/snap/frigate/current/config > {0}/config.ls.log'.format(TMP_DIR), throw=False)
        device.run_ssh('cp /var/snap/frigate/current/config/frigate/settings.json {0}/frigate.settings.json.log'.format(TMP_DIR), throw=False)
        device.run_ssh('cp /var/snap/frigate/current/config/authelia/config.yaml {0}/authelia.config.yaml.log'.format(TMP_DIR), throw=False)
        ...
        device.run_ssh('ls -la /snap/frigate > {0}/snap.ls.log'.format(TMP_DIR), throw=False)
        ...
        device.run_ssh('ls -la /data/frigate > {0}/data.ls.log'.format(TMP_DIR), throw=False)

        app_log_dir = join(artifact_dir, 'log')
        os.mkdir(app_log_dir)
        device.scp_from_device('/var/snap/frigate/common/log/*.log', app_log_dir)
...
def __log_data_dir(device):
    ...
    device.run_ssh('ls -la /data/')
    device.run_ssh('ls -la /data/frigate')


def test_storage_change_event(device):
    device.run_ssh('snap run frigate.storage-change > {0}/storage-change.log'.format(TMP_DIR))


def test_access_change_event(device):
    device.run_ssh('snap run frigate.access-change > {0}/access-change.log'.format(TMP_DIR))
